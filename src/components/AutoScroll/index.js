...
export const AutoScroll = ({ children, modal, special }) => {
  ...
  useLayoutEffect(() => {
    ...
    const containerElementListener = element?.parentNode?.addEventListener('scroll', handleScroll)
    const containerElement = element?.parentNode
    setParentElement(element)
    setContainerElement(containerElement)

    return () => {
      containerElementListener.removeEventListener('scroll', handleScroll)
    }
  })
