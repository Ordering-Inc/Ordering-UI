    ...
    <>
      ...
      <OptionStripeContainer>
        ...
        {token && !cardsList.loading && (
          <WrapperItems>
            <Button rectangle className='addcard' color='primary' onClick={() => setAddCardOpen(true)}>
              {t('ADD_PAYMENT_CARD', 'Add New Payment Card')}
            </Button>
            <ActionsModal>
              <Button rectangle onClick={() => onCancel()}>
                {t('CANCEL', 'Cancel')}
              </Button>
              <Button rectangle color='primary' onClick={() => onSelectCard(cardSelected)} disabled={!cardSelected}>
                {t('ACCEPT', 'Accept')}
              </Button>
            </ActionsModal>
          </WrapperItems>
